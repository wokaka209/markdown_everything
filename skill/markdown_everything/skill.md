...
```powershell
# PowerShell - 设置环境变量
$env:MARKITDOWN_PIP_MIRROR = "tsinghua"
.\manage_environment.ps1 -Command setup
```
...
如果网络状况较差，可以增加超时时间和重试次数：

```powershell
# PowerShell - 在脚本中修改（默认在脚本开头）
$script:PipTimeout = 180  # 超时时间（秒）
$script:PipRetries = 5   # 重试次数
```

```bash
# Bash - 环境变量方式
export MARKITDOWN_PIP_TIMEOUT=180  # 超时时间（秒）
export MARKITDOWN_PIP_RETRIES=5    # 重试次数
./manage_environment.sh setup
```
...
```powershell
# PowerShell - 超强网络优化组合
.\manage_environment.ps1 -Command setup -Mirror aliyun -UsePip
```

```bash
# Bash - 超强网络优化组合
export MARKITDOWN_PIP_MIRROR=aliyun
